...
from common.websocket import WsConnectionManager
from common.database.cache.redis import Cache
...
async def get_docs(credentials: HTTPBasicCredentials = Depends(doc_auth)):
    return get_swagger_ui_html(openapi_url='/CCC', title='Mono 문서')


@app.get('/BBB', include_in_schema=False)
async def get_redocs(credentials: HTTPBasicCredentials = Depends(doc_auth)):
    return get_redoc_html(openapi_url='/CCC', title='Mono 문서')
...
async def get_openapi_info(credentials: HTTPBasicCredentials = Depends(doc_auth)):
    openapi_schema = get_openapi(
        title='FastAPI Mono',
        version='1.0.0',
        description='FastAPI Mono 문서입니다.',
        routes=app.routes,
    )
